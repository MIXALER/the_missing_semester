...
模板参数的命名应当遵循对应的分类: 类型模板参数应当遵循[类型命名](#7.3 类型命名)的规则, 而非类型模板应当遵循[变量命名](#7.4 变量命名)的规则.
...
注释虽然写起来很痛苦，但对保证代码可读性非常重要。下面的规则描述了如何注释以及在哪儿注释。当然要记住：注释固然重要，但最好的代码应该本身就是文档。有意义的类型名和变量名，要远胜过用注释解释的含糊不清的名字。
...
[谷歌开源规范](https://zh-google-styleguide.readthedocs.io/en/latest/)
